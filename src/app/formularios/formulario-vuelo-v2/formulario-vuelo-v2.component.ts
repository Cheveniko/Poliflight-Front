...
export class FormularioVueloV2Component {
  ...
  constructor(
    ...
    private _vueloService: VuelosService //private studentService: StudentService
  ) {
    this.destino = '';
    this.origen = '';
    this.fecha = '';
    this.today = new Date().toISOString().split('T')[0];
    this.maxDate = new Date(new Date().getTime() + 360 * 24 * 60 * 60 * 1012)
      .toISOString()
      .split('T')[0];
  ...
  comprobarPasajeros(value: any) {
    ...
    if (
      this.pasajeros.adultos + this.pasajeros.ninos + this.pasajeros.infantes <
      10
    ) {
      if (
        10 -
